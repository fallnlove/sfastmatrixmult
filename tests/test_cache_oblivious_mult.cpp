...
class CacheObliviousMultTest : public ::testing::Test {
protected:
...
TEST_F(CacheObliviousMultTest, Correctness3x3) {
    using s_fast::Matrix;
    ...
    Matrix<int> return_val = s_fast::CacheObliviousMult(a, b);

    EXPECT_TRUE(res == return_val);
}

TEST_F(CacheObliviousMultTest, StressTest) {
    using s_fast::Matrix;
    using s_fast::Random;
    ...
    for (size_t _ = 0; _ < ItersCount(); ++_) {
        ...
        EXPECT_TRUE(s_fast::SimpleMultiplication(a, b) == s_fast::CacheObliviousMult(a, b));
    }
}
